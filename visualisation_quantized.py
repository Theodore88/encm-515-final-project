...
from quantize_helpers import quantize_array, dequantize_array
...
def plot_trajectory_3d(result: SimulationResult, out: str):
    ...
    ex = dequantize_array([s.x[0] for s in result.estimated_trajectory])
    ey = dequantize_array([s.x[1] for s in result.estimated_trajectory])
    ez = dequantize_array([s.x[2] for s in result.estimated_trajectory])
